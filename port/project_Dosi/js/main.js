...
$(document).ready(function() {
    // ****************하단배너 슬라이드 코드
    // 하단 배너의 Div
    var bannerSlide = $('.rollbanner');
    // 배너의 슬라이드 이미지 목록
    var bannerList = $('.rollbanner > ul > li')
    // 총 배너이미지의 개수
    var bannerTotalCount = bannerList.length;
    // 배너 이미지의 가로 너비
    var bannerWidth = Math.floor(bannerList.outerWidth());
    console.log(bannerWidth);

    // 최초 가로로 배치 한다.
    $.each(bannerList, function(index, item) {
        // 가로위치
        var tempX = index * bannerWidth;
        $(this).css({
            position: 'absolute',
            left: tempX,
            top: 0
    ...
    // 최소 가로 위치값
    var minX = - bannerWidth;
    // 최대 가로 위치값
    var maxX = bannerWidth * (bannerTotalCount - 1);
    // 모션 함수 실행 ->확인함
    // moveBanner();
    // 타이머를 만들자.
    var bannerTimer;
    ...
    $('.rollbanner_prev').mouseenter(function() {
        // 새로운 이동 모션을 막는다.
        // 새로운 타이머 생성을 죽인다.
        clearInterval(bannerTimer);
        // 새로운 모션도 막았다.
    ...
    $('.rollbanner_prev').mouseleave(function() {
        // 기본 모션방식으로 돌아간다.
        isMotion = true;
    ...
    $('.rollbanner_prev').click(function() {
        // 방향을 바꾼다.
        bannerDir = 'prev';
        // 모션 한번 실행
        nowSpeed = clickSpeed;
    ...
    $('.rollbanner_next').mouseenter(function() {
        isMotion = false;
        clearInterval(bannerTimer);
    });
    $('.rollbanner_next').mouseleave(function() {
        isMotion = true;
        nowSpeed = bannerSpeed;
    ...
    $('.rollbanner_next').click(function() {
        bannerDir = 'next';
        nowSpeed = clickSpeed;
    ...
    $('.rollbanner_pause').mouseenter(function() {
        isMotion = false;
        clearInterval(bannerTimer);
    });
    $('.rollbanner_pause').mouseleave(function() {
        isMotion = true;
        nowSpeed = bannerSpeed;
        clearInterval(bannerTimer);
        bannerTimer = setInterval(makeBannerTimer, bannerDelayTime);
    });
    $('.rollbanner_pause').click(function() {
        if (isAuto == true) {
            isAuto = false;
            $(this).css({
